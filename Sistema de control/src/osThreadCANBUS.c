...
/**
  * @brief  Thread de envio de mensaje 
  * @param  Ninguno
  * @retval Ninguno
  */
...
void osThreadCANBUS_CAN1_CALLBACK(CanRxMsg* RxMessage){
	...
	printf("Valores de recepcion ID = %d Dato recibido %d %d %d %d %d %d   ",RxMessage->StdId, RxMessage->Data[0], RxMessage->Data[1], RxMessage->Data[2], RxMessage->Data[3],RxMessage->Data[4], RxMessage->Data[5]);
	printf("Sender = %d   \r\n ",RxMessage->Data[7]);
...
void osThreadCANBUS_CAN1_StdFunc(uint32_t DATA1, uint32_t DATA2,uint32_t SENDER){
	...
	TAB_TRA.IMG_SEG    = ACTIVE ;
	TAB_TRA.IMG        = DATA1;
	TAB_TRA.IMG_CARRIER = DATA2;
...
void osThreadCANBUS_CAN1_InternalCom(uint32_t DATA1, uint32_t DATA2,uint32_t SENDER){
	...
	switch(DATA1){
		case Puerta_Abierta:
			switch(DATA2){
				case ACTIVE:
					GPIO_SetBits(LED_PUERTA_ABIERTA);
				  break;
